...
describe('extension upgrade: v2 -> v3', () => {
  ...
  it('splits the conflated section label into a rehearsal mark and a section name', () => {
    const doc: any = upgradeTabExtension(v2Document() as MnxStructure);

    expect(doc.global.measures[0]._x).toEqual({
      mnxLab: { rehearsal: { label: 'A' }, section: { label: 'Verse' } }
    });
    // A measure with only one of the two gets only that one.
    expect(doc.global.measures[1]._x).toEqual({ mnxLab: { section: { label: 'Chorus' } } });
  ...
  it('leaves an already-v3 document untouched, by identity', () => {
    const doc = {
      mnx: { version: 1 },
