...
/** The committed rollup, over every scenario — which sweeps any not swept yet. */
function buildReport() {
  const results = corpus.map(sweep);
...
describe('destruct sweep (element-ops campaign item 2)', () => {
  for (const scenario of corpus) {
    it(`${scenario.id}: no removal breaks an invariant`, () => {
      const broken = sweep(scenario).elements.filter(row => row.verdict === 'broken');
      expect(
