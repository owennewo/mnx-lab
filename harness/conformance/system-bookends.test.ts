...
function assertBookends(layout: LayoutResult) {
  ...
  if (leading?.kind === 'rect' && trailing?.kind === 'rect' && first && last) {
    ...
    expect(trailing.x + trailing.w).toBeLessThanOrEqual(layout.usedWidthSp + 0.001);
    for (const label of layout.primitives.filter(p => p.kind === 'text' && tokens(p).includes('staff-label'))) {
      if (label.kind === 'text') expect(label.x).toBeLessThan(leading.x);
    }
  }
