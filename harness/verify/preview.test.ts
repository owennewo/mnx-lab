...
// `node harness/verify/verify-scenarios.mjs …` command to copy-paste.
//
...
import { loadCorpus, createContext, checkScenario, ROOT } from './check-scenarios.mjs';
// @ts-expect-error — plain .mjs module without type declarations
import { loadSpecExamples } from '../../spec/tools/specSource.mjs';
...
<div id="bar">
  ...
  <code id="cmd">node harness/verify/verify-scenarios.mjs</code>
...
function refresh() {
  ...
  cmd.textContent = 'node harness/verify/verify-scenarios.mjs' + (ids.length ? ' ' + ids.join(' ') : '');
}
