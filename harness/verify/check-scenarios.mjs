...
//
// Usage: node harness/verify/check-scenarios.mjs
// Also imported as a library by harness/conformance/scenarios.test.ts.
...
export function createContext() {
  ...
  const mnxSchema = readJson(path.join(ROOT, 'spec', 'mnx-schema.json'));
  const extSchema = readJson(path.join(ROOT, 'spec', 'mnx-lab-extensions.schema.json'));
  ...
  const proposedPath = path.join(ROOT, 'spec', 'mnx-schema.proposed.json');
...
export function checkScenario(scenario, ctx) {
  ...
  const expectedOrigin = scenario.ns === 'spec' ? 'mirrored' : 'local';
  if (meta.origin === undefined) {
    fail(`missing "origin" — scenarios under ${scenario.ns}/ declare "${expectedOrigin}"`);
  } else if (meta.origin !== expectedOrigin) {
    fail(
      `origin "${meta.origin}" contradicts its location: ${scenario.ns}/ is ` +
        (scenario.ns === 'spec' ? 'generated by sync:spec (mirrored)' : 'ours (local)')
    );
  }
  if (meta.origin === 'mirrored' && meta.schema === 'proposed') {
    fail('origin "mirrored" cannot declare schema "proposed" — the pin is always upstream');
  }
  ...
  if (wantsProposed) {
    if (!meta.proposal) {
      fail(
        'declares "schema": "proposed" but names no "proposal" topic — every proposal ' +
          'scenario belongs to a spec/proposals/<topic>/ evidence bundle'
      );
    } else if (!fs.existsSync(path.join(ROOT, 'spec', 'proposals', meta.proposal))) {
      fail(`proposal topic "${meta.proposal}" has no spec/proposals/${meta.proposal}/ bundle`);
    }
  } else if (meta.proposal !== undefined) {
    fail('"proposal" is set but schema is not "proposed"');
  }
  if (wantsProposed && !ctx.proposedSchema) {
    fail(
      'declares "schema": "proposed" but spec/mnx-schema.proposed.json is absent — ' +
