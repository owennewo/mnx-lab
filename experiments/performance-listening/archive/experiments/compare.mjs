...
const output = path.resolve(
  process.argv[3] ?? path.join(bench, "output/run-v1"),
);
const limit = Number(
  process.argv.find((a) => a.startsWith("--limit="))?.split("=")[1] ?? Infinity,
);
if (fs.existsSync(output))
  throw Error(`Refusing to overwrite ${output}; choose a new run directory.`);
const manifestFile = path.join(audio, "manifest.json"),
  manifest = readJSON(manifestFile),
  config = readJSON(path.join(bench, "experiments/baseline.json"));
...
for (const record of manifest.records) {
  ...
  if (
    loaded.hash !== record.audioHash ||
    loaded.sampleRate !== config.sampleRate
  )
    throw Error("Audio hash/sample rate mismatch: " + record.file);
  samples.set(record.file, loaded.samples);
}
const templates = new Map(
  manifest.records
    .filter((r) => r.split === "template" && r.preset === config.templatePreset)
    .map((r) => [r.actual[0].pitch, samples.get(r.file)]),
);
const harmonic = createScorer(harmonicDictionary(config), "harmonic", config);
const template = createScorer(
  templateDictionary(templates, config),
  "template",
  config,
);
const provenance = {
  ...
  algorithms: {
    harmonic:
      "v1: twelve 1/h harmonics, Gaussian bin width .8, greedy matching pursuit",
    template:
      "v1: isolated archtop .45–.75s mean magnitude, nonnegative coordinate descent",
    shared:
      "Hann trailing FFT; two-frame activity gate; no restrike detector; coefficients uncalibrated",
  },
  audioDirectory: path.relative(output, audio),
...
let neural,
  failures = 0;
try {
  neural = await createNeural(config);
  provenance.neural = neural.provenance;
} catch (error) {
  provenance.neural = { status: "unavailable", reason: String(error) };
  failures++;
}
...
for (const record of records) {
  const pcm = samples.get(record.file),
    duration = pcm.length / config.sampleRate;
  for (const [strategy, scorer] of [
    ["harmonic", harmonic],
    ["template", template],
    ["neural", null],
  ]) {
    for (const mode of [
      "offline",
      ...config.streamChunks.map((n) => `stream-${n}`),
    ]) {
      const base = {
        fixture: record.id,
      ...
      if (strategy === "neural" && (mode !== "offline" || !neural)) {
        results.push({
          ...
          status: "unavailable",
          reason: !neural
            ? provenance.neural.reason
            : "Upstream adapter requires whole-file context; causal neural inference is not implemented.",
        });
        continue;
      }
      try {
        const detection =
          strategy === "neural"
            ? await neural.detect(pcm)
            : detectDSP(
                pcm,
                scorer,
                config,
                mode === "offline" ? pcm.length : Number(mode.split("-")[1]),
              );
        ...
        results.push({ ...base, status: "failed", reason: String(error) });
        failures++;
      }
    }
  }
  ...
  console.log(
    `${record.preset}/${record.id}: ${results.length} outcomes, ${failures} failures`,
  );
}
neural?.dispose();
function summarize(selected) {
  ...
  return {
    runs: good.length,
    unavailable: selected.filter((r) => r.status === "unavailable").length,
...
for (const result of results) {
  const key = `${result.strategy}/${result.mode}/${result.preset}`;
  groups[key] ??= [];
...
const summary = {
  ...
  categories: Object.fromEntries(
    [
      ...new Set(
        results.map((r) => `${r.strategy}/${r.mode}/${r.preset}/${r.category}`),
      ),
    ].map((k) => [
      k,
      summarize(
        results.filter(
          (r) => `${r.strategy}/${r.mode}/${r.preset}/${r.category}` === k,
        ),
      ),
    ]),
  ),
