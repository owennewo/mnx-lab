...
export class AttackEvidence {
  constructor(config) {
    ...
    this.bands = Array.from({length: config.midiMax - config.midiMin + 1}, (_, i) => {
      const bins = [];
      for (let h = 1; h <= 6; h++) {
        const center = Math.round(hz(config.midiMin + i) * h * config.fftSize / config.sampleRate);
  ...
  push(magnitude, rms, time) {
    this.bands.forEach((bins, i) => {
      ...
      const above = rms >= this.config.rmsFloor && energy > 0 && rise / energy >= this.config.fusion.threshold;
  ...
  permits(i, time) {
    return this.crossing[i] > this.consumed[i] &&
