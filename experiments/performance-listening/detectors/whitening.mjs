...
export function createWhitener(config) {
  ...
  if (!(w.strength > 0 && w.strength <= 1) || !(w.floor > 0 && w.floor <= 1) || !(w.maxGain >= 1 && Number.isFinite(w.maxGain)))
    throw Error('Invalid whitening configuration');
  const centers = Array.from({ length: 32 }, (_, b) => 229 * (10 ** ((b + 1) / 21.4) - 1));
  const size = config.fftSize / 2 + 1;
  const bands = Array.from({ length: 30 }, (_, i) => {
    const b = i + 1, entries = [];
    for (let k = 0; k < size; k++) {
      const f = k * config.sampleRate / config.fftSize;
      const weight = f <= centers[b] ? (f - centers[b - 1]) / (centers[b] - centers[b - 1]) : (centers[b + 1] - f) / (centers[b + 1] - centers[b]);
  ...
  return (magnitude) => {
    const sigma = bands.map(entries => Math.sqrt(entries.reduce((s, [k, weight]) => s + weight * magnitude[k] ** 2, 0) / config.fftSize));
    const peak = Math.max(...sigma);
    if (peak === 0) return magnitude;
    const gains = sigma.map(s => Math.min(w.maxGain, Math.max(s / peak, w.floor) ** -w.strength));
