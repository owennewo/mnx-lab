...
describe('online-time-warp@1', () => {
  it('follows a rendered score at the handed tempo', () => {
    const run = execute(onlineTimeWarp1, scale, { bpm: 60, unit: 'quarter' }, audio);
  ...
  it('reports unsupported for silence and for audio that is not the handed score', () => {
    const silent = execute(onlineTimeWarp1, scale, { bpm: 60, unit: 'quarter' }, new Float32Array(audio.length));
    expect(silent.record.every(d => d.kind === 'unsupported')).toBe(true);
    const other = execute(onlineTimeWarp1, oneBar, { bpm: 60, unit: 'quarter' }, audio.slice(4 * 48000));
  ...
  it('decides from the prefix alone', () => {
    ...
    expect(prefix(execute(onlineTimeWarp1, scale, tempo, changed).record)).toBe(prefix(execute(onlineTimeWarp1, scale, tempo, audio).record));
  });
});
