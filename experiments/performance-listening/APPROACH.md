...
- A **click-guided** human performance establishes intended timing, not actual note
  timing or pitch. Actual performance labels still require independent evidence;
  small deviations cannot be assumed.
...
performances. More synthetic data cannot remove that dependency, and the loop cannot
promise indefinite improvement.
