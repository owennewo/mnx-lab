...
const sources = [
  ['contract', 'contracts/research-contract-1-draft.md'],
...
<article><span class="tag">Draft contract · human review pending</span><h1>Start with four bars</h1>
<p>The Winner Takes It All first, then Dust in the Wind. Local four-bar audio review clips, score links and anchor checks are ready. This is preparation for the next assessment, not experiment 002 and not evidence that a listener passed.</p>
<p><a href="file:///home/williao/dev/mnx-listening-data/real-evidence-01/index.html">Open the private listening and anchor-review packet</a> on this machine. The audio and scores stay outside the repository.</p>
<h2>The decisions to review</h2><ul>
<li>Known start; solo guitar through a microphone on this laptop; timing within 80–120% of the supplied nominal tempo.</li>
<li>Proposed gates: at least 95% correct supported positions and 95% correct rejection; 98% decision coverage; at most 5% wrong/false exposure, with no episode over 0.5 s.</li>
...
<li>No source is yet a golden. Human label checks and an uncertainty-aware instrument are still required. Independent reserved and microphone acceptance evidence are required for formal retention and qualification.</li></ul>
<p>The full draft below also specifies recovery, uncertainty, minimum worthwhile improvement, budgets, stopping and the separate Studio integration requirements. Scope confirmation is already recorded; these numerical choices still need explicit approval.</p></article>
${sources.map(source => `<details${source.id === 'contract' ? ' open' : ''}><summary>${source.id === 'contract' ? 'Research contract 1 — full draft' : 'Evidence preparation, findings and reproduction'}</summary>${markdown(source, here)}</details>`).join('\n')}
