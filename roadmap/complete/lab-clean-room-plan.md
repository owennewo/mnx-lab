...
> scenario-library structure (`../inprogress/lab-04-scenario-library.md`) is the living corpus
> contract; the **monorepo package split was not adopted** (`../superseded/lab-02-architecture.md`).
...
Reusable design ideas come from `../../SVG_RENDERING_ENGING.md`, `../../research/`, `core-musicxml.md`,
and `../../schemas/`.
...
fixtures, the app's content, and executable documentation of MNX (see `../inprogress/lab-04-scenario-library.md`).
...
1. **Monorepo of packages.** Each capability is its own package with its own tests; apps
   compose them. (See `../superseded/lab-02-architecture.md`.) *— not adopted; the app stayed a
   single `mnx-lab` in `src/`.*
2. **Beachhead = scenario library + gallery.** Ship a corpus of valid MNX and an app that
   validates and renders it — no editor, no player, no server. (See `lab-03-rollout.md`.)
...
|---|-----|---------------------|-----------|
| 0 | `lab-00-vision.md` | Why build this, for whom, and what is explicitly *not* in scope? | provisional |
| 1 | `lab-01-principles.md` | What rules must never be broken? | settled-ish |
| 2 | `../superseded/lab-02-architecture.md` | What are the modules and the contracts between them? | provisional |
| 3 | `lab-03-rollout.md` | What do we build first, and what is "not yet"? | provisional |
| 4 | `../inprogress/lab-04-scenario-library.md` | **The heart:** how is the scenario corpus structured? | provisional |
| – | `lab-module-specs.md` | How is each module built? (written just-in-time) | per-file |
| – | `decisions/*.md` | Why did we choose X over Y? (ADRs — never created) | append-only |

## Two rules that keep this honest

- **Contract-first.** A module spec in `lab-module-specs.md` may not be written until that module's
  contract exists in `../superseded/lab-02-architecture.md`. This keeps the pieces decoupled and reusable.
