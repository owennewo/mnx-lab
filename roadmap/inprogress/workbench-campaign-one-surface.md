...
- **2026-08-31 — item 11 split into 11a/11b** (user decision, census-grounded).
  ...
  **11b** = the verbs, whose census supports a strong thesis: every verb
  family is already a rung-generic gesture (insert sibling = `insertAtRung`
  with the cursor following — "create sibling and move to it" is one verb,
  not six tiles; delete/clear = `Del`'s ladder; closures and history have
  their keys). 11b's real work: the one missing generic (construct the
  missing child — `new-voice` as a partMeasure `voice` declaration), four
  blocked-residue dispositions (`arpeggio`'s blocker may be stale —
  `setPositioned arpeggio` exists), the popover-frame demolition, and the
  shell's retirement with discoverability re-homed.
- **2026-08-31 — item 6 (lyrics) phase 3 built; the item closes.** The pass
  model landed as designed: `src/model/passes.ts` linearizes the repeat
  structure once for three consumers (player timeline, verse↔pass
  resolution, diagnostics), and the blue bound reads as one rule — verse
  ordinal *k* on a bar whose sounding passes lack *k* — silent without
  repeat structure (the hymn convention), translations exempt via
  per-language ordinals, surfaced non-blocking in the text editor.
  Learnings: the walk needed an explicit "ending exit" reset (music after a
  resolved strain is pass 1 again — caught writing the volta test, not the
  code); and the campaign's §6 goldens contract cleanly split what shipped
  from what deferred — renderer default labels and an engine badge move
  goldens, so they wait for the player rather than stretching a surface
  item. Item 11 (the tray, as 11a/11b) is all that remains.
...
- **2026-08-31 — item 9 (part) built.** Construction landed as declaration
  ...
  rename arm, so typing a name ADDED a part; `PartDeclaration` gained `name`,
  and `setStaffKind` got the parts[0] widening (third member of the family).
  The document rung earned its first pills (the support flags) and words.
  Learnings: count tiles by grep, not memory (a fifth tile — `capo` at
  partMeasure — surfaced via the `not in registry` assertion; make it
  standard), and ask of every popover tile "what does typing into this
  actually fire?" — the label can promise an op that does not exist.
- **2026-08-31 — item 6 (lyrics) phase 2 built.** The paste-and-tweak text
  surface: `src/edit/lyricText.ts` (walk/serialize/parse/diff, pure and
  conformance-tested), a new `applyLyricPlan` intent (note-addressed edits in
  one batch — the clipboard's materialized-plan shape; undo is one gesture,
  traces record syllables), and `<mnx-lyric-text-editor>` as a modal over the
  score. **`Shift+L` lives again** — the earmark cashed as designed: it opens
  the editor, and the campaign's freed-keys ruling stands unbroken for every
  other item. Learnings: the `SelectionContext.preview` channel is the right
  vehicle for any overlay's transient highlight (second consumer after the
  tray); a modal with a `<textarea>` needs no PENDING_PRECEDENCE wiring at
  all — `keyScope.isTextEntry` already yields the page keymap; and
  `smoke:inspector` absorbed a lyric-editor lap cheaply because its probe
  helpers are generic over shadow roots.
- **2026-08-31 — item 6 (lyrics) phase 1 built.** The popover is retired: verse-line
  metadata lands as document-rung `line` pills (recited in `lineOrder`, unlisted ids
  after — the renderer's own stacking rule), the typed `line …`/`no line …` arms
  route through `parseLyric` at the document rung, and the event/note rung signposts.
  Contract §5's one exception is now real: **`Shift+L` is freed but earmarked** — the
  unbound slot in `keymap.ts` records that phase 2's lyric text surface reopens it as
  an accelerator. Two campaign firsts: a **band retirement** (the note rung's `text`
  band emptied with its only tile — deleted, since an empty band is the conformance
  join's own red flag), and an item left 🔧 in `inprogress/` after its sweep (phases
  2–3 live under the same doc). Sweep learning confirmed from item 7: KNOWN_TWINS
  billed nothing, and crediting `setLyricLine`/`removeLyricLine` to `rungInspector`
  BEFORE deleting `SURFACE_INTENTS.lyricPopover` is what keeps the construct-traces
  join green through the change.
- **2026-08-31 — item 6 (lyrics) design settled.** Both investigations
  closed in one conversation, doc rewritten to record it. Lines × repeats:
  **derived, no data** — a shared pass-linearization walk in `model/`
  (player, entry, diagnostics all consume it), pass → line by ordinal within
  a language group of `lineOrder`. WYTIWYG lane: **not built** — replaced by
  a paste-and-tweak modal text editor (LilyPond-kin format: hyphen/underscore
  runs, `~` elision, `|` bar checks, order-free `lang pass:` headers) whose
  apply diffs into ordinary ops through the one funnel. The Shift+L
  exception is confirmed but deferred: freed at the phase-1 sweep, earmarked
  to reopen the text editor when phase 2 lands. Found along the way: MNX
  cannot encode a melisma extender (`event-lyric-line` is `{text, type}`
  only) — a candidate spec-loop topic, deliberately not filed.
