...
[studio-sync-rederive](../complete/studio-sync-rederive.md). The owner's question — *"hasn't my three
...
[core-editor-element-promotion](../complete/core-editor-element-promotion.md), *Slice 1*. The owner's
...
[core-editor-element-promotion](../complete/core-editor-element-promotion.md), *Slices 2–3*.
...
### 2026-09-20 — entry 11: item 4 closed, item 6 skipped, item 9 filed

Three owner decisions from one conversation, no code. **Item 4 is complete** with its two
hands-on checks (the click against a real YouTube clock; the sync bar on touch) still being
made — the owner keeps testing on the live site and did not want the campaign held on them.
...
Learning: the pickup note said the post-deploy checks could only be seen on the real site.
Wrong — YouTube plays locally under `wrangler dev`, so every one of them can be walked before a
deploy. Corrected there.
