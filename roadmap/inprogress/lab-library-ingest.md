...
**Pending: production data ingest and landing.** Automatic approval review rejected
the upload twice, including after being shown the campaign's exact item 3 contract.
It requires the user to explicitly approve sending the two cached pieces (Blues Run The
Game and Sweet Child O Mine), their scores, recordings and metadata, to the user's
authenticated library at `https://mnx-lab.totai.uk`. Neither rejected command executed;
no cache data has been uploaded to production. The committed worktree is retained until
that approval permits ingest, replay verification and the fixed landing sequence.
