...
### proposed/
- **[core-element-ops-destruct-sweep.md](proposed/core-element-ops-destruct-sweep.md)** —
  campaign item 2 of [core-campaign-element-ops.md](inprogress/core-campaign-element-ops.md):
  **the destructibility sweep at corpus scale** — item 1's reverse walk over all 106
  scenarios and every kind of ink in them. A harness item, so its agreement block is four
  decisions rather than the contract's op/key/rung. **What counts as an element**: a
  document walk per kind with the renderer's own **ink census** as its completeness oracle
  (5,406 primitives, 63 classes across the goldens — and only noteheads/fret numbers carry
  a `sourceId`, which is why the walker cannot simply be derived from the render output),
  with element-vs-structure drawn by the repo's existing *encode the choice, not the
  consequence* rule. **The verdicts**: two independent axes (addressed? removed?) where
  only a broken oracle reddens a build — `no-op` and `unaddressable` are the campaign's
  scoreboard, not failures, and destruct notably has **no** expected-unreachable class.
  **The oracles widened**: validity becomes relative (the five invalid-by-design exhibits
  start invalid), references grow past ties to slurs, technique and the 98 beam→event
  refs, and a new **surviving-document** check finally asserts the campaign's
  "byte-identical except forced cascades". **The artifact**: a committed report of rollups
  + findings regenerated by `npm run sweep:destruct`, with `npm test` failing on drift in
  either direction — so a later item's removal verb shows up as rows flipping `no-op` →
  `removed`. Measured cost: 10–14 ms per element, ≈9–11 s for the corpus's 801 notes.
