...
fixtures, the app's content, and executable documentation of MNX (see `04-scenario-library.md`).
...
1. **Monorepo of packages.** Each capability is its own package with its own tests; apps
   compose them. (See `02-architecture.md`.)
...
| 2 | `02-architecture.md` | What are the modules and the contracts between them? | provisional |
| 3 | `03-rollout.md` | What do we build first, and what is "not yet"? | provisional |
| 4 | `04-scenario-library.md` | **The heart:** how is the scenario corpus structured? | provisional |
| – | `modules/*.md` | How is each module built? (written just-in-time) | per-file |
| – | `decisions/*.md` | Why did we choose X over Y? (ADRs, grow over time) | append-only |

## Two rules that keep this honest

- **Contract-first.** A module spec in `modules/` may not be written until that module's
  contract exists in `02-architecture.md`. This keeps the pieces decoupled and reusable.
